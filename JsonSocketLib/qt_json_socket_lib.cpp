...
{
    disconnect (sock__, &QAbstractSocket::disconnected, this, &Qt_Json_Socket_Lib::disconnect_slot);
    delete sock__;
}
...
bool Qt_Json_Socket_Lib::make_QTcpSocket()
{
    ...
    //disconnect 시그널 연결
    connect(sock__, &QAbstractSocket::disconnected, this, &Qt_Json_Socket_Lib::disconnect_slot);
...
bool Qt_Json_Socket_Lib::send_QByteArray(QByteArray arr)
{
    try
    {
        ...
        sock__->write (byte_Size.toUtf8 (), DEFAULTSRESOURCESIZE);
...
QByteArray Qt_Json_Socket_Lib::recv_QByteArray()
{
    try
    {
        ...
        //버퍼 사이즈를 읽음
        //만약 소켓이 읽는 타임아웃이 될경우
        if (sock__->waitForReadyRead (recv_timeout__) == false)
        {
        ...
        QByteArray recv_siz = sock__->read (DEFAULTSRESOURCESIZE);
...
bool Qt_Json_Socket_Lib::send_Json(QJsonObject obj)
{
    ...
    if (obj.isEmpty() )
    {
        qDebug() << "[Info] : obj is empty send fasle";
        return false;
    }
...
void Qt_Json_Socket_Lib::connect_server(const QString& a, qint64 port)
{
...
bool Qt_Json_Socket_Lib::is_connect()
{
    if (sock__->isOpen() )
    {
        return true;
